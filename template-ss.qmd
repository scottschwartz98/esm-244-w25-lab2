...
f1 <- mass~bill_l +bill_d +flip_l + sex +species + island
mdl1<-lm(f1,data=penguins_clean)
summary(mdl1)


f2 <-mass ~ bill_l + bill_d + flip_l + sex + species
...
Pseudocode:
with a 10 fold we need to select how much training data we will use, (10 folds)
for loop:
  apply the model to each training set
random sampling 
we need to create a way to score the outputs of the models - what metric?
  root mean squared error
  lets make a function for RMSE
  make predictions on the test set with fitted training model
close loop:

summarize our RMSE (which model on average was the best)
...
kfold_lm1 <- lm(f1,data=kfold_train_df)
  ...
  pred_df <-  kfolds_test_df |>
    mutate(model1 = predict(kfold_lm1, kfold_test_df))
...
kfold_cv<-function(i,df,formula){
  ...
  kfold_rmse <-kfold_pred_df |>
    summariz(rmse_md=calc_rmse(md,mass))
